...
char *myName = "Zane_Partain";

/*******************************************************
  kfork() creates a child process; returns child pid.
  When scheduled to run, child struct proc resumes to body();
********************************************************/
...
int body(char* myname)   // process body function
{
  int c;
  char cmd[64];
  if(running->pid == 1){ //set global INIT ptr to P1
    INIT = running;
  }
  printf("%s", myName);
  putchar('\n');
...
{
  int  i;
  struct proc *p = dequeue(&freeList);
  if (!p){
     printf("no more proc\n");
     return(-1);
  }
  ...
  **********************************************************/
  ...
  p->ksp = &(p->kstack[SSIZE - 9]);  // struct proc.ksp -> saved eflag 
  enqueue(&readyQueue, p);           // enter p into readyQueue

  if(running->child){
    struct proc *childPtr = running->child;
    ...
    running->child = p;           // assign new proc of kfork as a child to running proc      

  p->ppid = running->pid;         // set new proc ppid to running proc
  p->parent = running;
  return p->pid;
}
int do_wait(){
